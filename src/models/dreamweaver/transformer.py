...
class TransformerDecoderBlock(nn.Module):
    ...
    def __init__(self, max_len, d_model, num_heads, dropout=0., gain=1., is_first=False, use_sln=False):
        super().__init__()
        self.use_sln = use_sln
        
        self.is_first = is_first
        self.self_attn_layer_norm = nn.LayerNorm(d_model) if not use_sln else SLN(d_model)
        ...
        self.encoder_decoder_attn_layer_norm = nn.LayerNorm(d_model) if not use_sln else SLN(d_model)
        self.encoder_decoder_attn = MultiHeadAttention(d_model, num_heads, dropout, gain)
        
        self.ffn_layer_norm = nn.LayerNorm(d_model) if not use_sln else SLN(d_model)
        self.ffn = nn.Sequential(
            linear(d_model, 4 * d_model, weight_init='kaiming'),
            nn.ReLU(),
            linear(4 * d_model, d_model, gain=gain),
    ...
    def forward(self, input, encoder_output, w=None):
        """
        ...
        if self.is_first:
            input = self.self_attn_layer_norm(input) if not self.use_sln else self.self_attn_layer_norm(input, w)
            x = self.self_attn(input, input, input, self.self_attn_mask[:T, :T])
            input = input + x
        else:
            x = self.self_attn_layer_norm(input) if not self.use_sln else self.self_attn_layer_norm(input, w)
            x = self.self_attn(x, x, x, self.self_attn_mask[:T, :T])
            input = input + x
        
        x = self.encoder_decoder_attn_layer_norm(input) if not self.use_sln else self.encoder_decoder_attn_layer_norm(input, w)
        x = self.encoder_decoder_attn(x, encoder_output, encoder_output)
        input = input + x
        
        x = self.ffn_layer_norm(input) if not self.use_sln else self.ffn_layer_norm(input, w)
        x = self.ffn(x)
        return input + x
...
class TransformerDecoder(nn.Module):
    ...
    def __init__(self, num_blocks, max_len, d_model, num_heads, dropout=0., use_sln=False):
        super().__init__()
        ...
        if num_blocks > 0:
            ...
            self.blocks = nn.ModuleList(
                [TransformerDecoderBlock(max_len, d_model, num_heads, dropout, gain, is_first=True, use_sln=use_sln)] +
                [TransformerDecoderBlock(max_len, d_model, num_heads, dropout, gain, is_first=False, use_sln=use_sln)
                 for _ in range(num_blocks - 1)])
        else:
            self.blocks = nn.ModuleList()
        
        self.layer_norm = nn.LayerNorm(d_model) if not use_sln else SLN(d_model)
    ...
    def forward(self, input, encoder_output, w=None):
        ...
        """
        for block in self.blocks:
            input = block(input, encoder_output, w)
        
        return self.layer_norm(input) if not self.use_sln else self.layer_norm(input, w)
